...
from quantum.plugins.janus import ofp_service_type
from quantum.plugins.janus.common import config
from quantum.plugins.janus.db import api_v2 as db_api_v2

LOG = logging.getLogger(__name__)


class JanusQuantumPluginV2(db_base_plugin_v2.QuantumDbPluginV2,
    ...
    def delete_network(self, context, id):
        ...
        with session.begin(subtransactions=True):
            ...
            #   q-svc deleting network before q-dhcp can unregister port.
            #   In the future perhaps Janus can be modified to allow this.
            bridge = self.integration_bridge
            if ip_lib.device_exists(bridge):
                ovs_br = ovs_lib.OVSBridge(bridge, self.root_helper)
            ...
            for port in ports:
                if port['device_owner'] in \
                    ...
                    vif = ovs_br.get_vif_port_by_id(port['id'])
                    self.client.deletePort(id, datapath_id, vif.ofport)
                    self.client.delMAC(id, vif.vif_mac)
    ...
    def delete_port(self, context, id, l3_port_check=True):
        ...
        return super(JanusQuantumPluginV2, self).delete_port(context, id)
